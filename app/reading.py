...
# where=variable declared by template to redirect back from where pagination was initiated on)
...
# rendering template for main search in database, shows information of all databases except game_suggestions
@app.route('/browse', methods=['POST', 'GET'])
def browse():
    # initialization of required standard variables required to show results and pagination without choosing choices
    ...
    # render template with name chosen in browse form
        ...
        return render_template('browse.html',
                                ...
                                results=results,
                                PAGE_NUMBER=session['PAGE_NUMBER'])
    # render template with nothing chosen in browse form requires initial initialization of variables in the start of function
    else:
        return render_template('browse.html',
                                not_selected=True,
                                review_ratings=review_ratings,
...
@app.route('/top_games')
def top_games():
    # initialization for all variables required for upating the game keys required to show and update results
    ...
    # updating game keys in order to show correct averages, ratings and total ratings
    ...
    # sort by average, rating and total rating in order to show results correctly on template
...
# rendering templates for admin sites, check if admin is logged in before rendering results                    
